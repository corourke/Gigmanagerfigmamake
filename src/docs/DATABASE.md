...
| is_template | BOOLEAN | Whether this kit is a template for duplication (default false) |
| created_by | UUID | Reference to users.id (informational only, no relation) |
| updated_by | UUID | Reference to users.id (informational only, no relation) |
| created_at | TIMESTAMPTZ | Record creation timestamp |
| updated_at | TIMESTAMPTZ | Record last update timestamp |
...
- Templates allow creating standardized kit configurations that can be duplicated
